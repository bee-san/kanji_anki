...
internal fun StudyQueueItem.toEntity(): StudyItemEntity = StudyItemEntity(
    kanji = kanji,
    ...
    learningStep = learningStep,
    writingLevel = writingLevel,
    recognitionStage = rung.toLegacyRecognitionStage(),
    consecutiveFailedRecognitionDays = realAgainStreak,
    lastFailedRecognitionDay = lastRealReviewDueAtMillis,
    writingRemediationPending = if (rung == StudyRung.WRITE_KANJI) 1 else 0,
    suppressedByTaskType = suppressedByTaskType,
    suppressedAt = suppressedAtMillis,
    matureIntervalDays = matureIntervalDays,
    answerSignature = answerSignature,
    typingMeaningMemory = memories.typingMeaningMemory.encode(),
    meaningKanjiMemory = memories.meaningKanjiMemory.encode(),
    kanjiMeaningMemory = memories.kanjiMeaningMemory.encode(),
    fontMeaningMemory = memories.fontMeaningMemory.encode(),
    wordReadingMemory = memories.wordReadingMemory.encode(),
    writingRemediationMemory = memories.writingRemediationMemory.encode(),
    rung = rung.wireName,
    phase = phase.wireName,
    realPassStreak = realPassStreak,
    realAgainStreak = realAgainStreak,
    lastRealReviewDueAt = lastRealReviewDueAtMillis,
    similarKanjiMemory = memories.similarKanjiMemory.encode(),
    activeToken = activeToken,
