...
| 6.5 | major | The handwriting pad is invisible to TalkBack (no contentDescription/semantics; escape hatches conditional). | **DEFERRED**: accessibility pass on `DrawingPadView`. |
...
| 6.10 | minor | Choice-feedback text is fixed white on teal (~2.6:1 contrast, below WCAG 3:1); pre-Compose window background follows system night mode instead of the in-app theme. | **DEFERRED**: theming tokens follow-up. |
