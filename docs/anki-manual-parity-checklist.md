...
8. Import/sync should document source-of-truth and conflict boundaries.
   ...
   Live boundary verification (2026-07-10): the real AnkiDroid 2.24.0 gate passed `OK (61 tests)` against the copied 7,000+ note collection. A non-destructive probe called `resolver.update` on a real card URI with that card's existing queue value. The provider rejected it with `IllegalArgumentException` (`updatedRows=-1`), and a reread confirmed queue `2` was unchanged. Direct card unsuspend is therefore not a supported provider operation in Kani.
