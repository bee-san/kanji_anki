...
| Bury / siblings | `StudySessionSelector` collapses active items by kanji family and chooses one active family item. | This approximates sibling suppression, but does not expose Anki's bury-new, bury-review, or bury-interday controls. | Add focused tests if parity with Anki bury settings becomes a product goal. |
...
1. Add tests or documentation for the intended sibling/bury behavior if users report seeing repeated same-kanji family prompts.
