...
class BundledSqlDriver(
    ...
    override fun openConnection(mode: SqlConnectionMode): SqlConnection {
        ...
        return translateSqlFailure("open SQLite connection") {
            val connection = BundledSqlConnection(driver.open(path), mode)
            try {
...
private class BundledSqlConnection(
    ...
    override fun prepare(sql: String): SqlStatement {
        ...
        return translateSqlFailure("prepare statement") {
            BundledSqlStatement(delegate.prepare(sql))
        }
    }
...
private class BundledSqlStatement(
    ...
    override fun query(): SqlRows {
        checkOpen()
        return BundledSqlRows(delegate)
    }
...
private class BundledSqlRows(
    private val statement: AndroidxStatement,
) : SqlRows {
    private var closed = false
    private var positioned = false

    override val row: SqlRow = BundledSqlRow(statement) {
