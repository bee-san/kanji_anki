...
public class GitHubReleaseMetadataParserTest {
    @Test
    public void parsesLatestReleaseAssets() {
        ...
        GitHubReleaseMetadata info = GitHubReleaseMetadataParser.parseLatest(json);
    ...
    public void malformedReleaseJsonFallsBackToEmptyValues() {
        GitHubReleaseMetadata empty = GitHubReleaseMetadataParser.parseLatest(null);

        assertEquals("", empty.tagName());
        assertEquals("", empty.htmlUrl());
        assertTrue(empty.assets().isEmpty());

        GitHubReleaseMetadata malformed = GitHubReleaseMetadataParser.parseLatest(
                "{"
                        + "\"tag_name\":123,"
                        + "\"html_url\" false,"
                        + "\"assets\":\"not-an-array\","
    ...
    public void parserHandlesEscapesBrokenUnicodeAndUnclosedStrings() {
        GitHubReleaseMetadata escaped = GitHubReleaseMetadataParser.parseLatest(
                "{"
                        + "\"tag_name\":\"v0.4.0\","
        ...
        GitHubReleaseMetadata unclosed = GitHubReleaseMetadataParser.parseLatest("{\"tag_name\":\"v0.4.1");
    ...
    public void parserSkipsNestedStringsWhileFindingKeysAndObjects() {
        GitHubReleaseMetadata info = GitHubReleaseMetadataParser.parseLatest(
                "{"
                        + "\"assets\":["
    ...
    public void parserHandlesMissingValuesUnclosedArraysAndUnknownEscapes() {
        GitHubReleaseMetadata missingValues = GitHubReleaseMetadataParser.parseLatest(
                "{\"tag_name\":   ,\"assets\":   }"
        );
        GitHubReleaseMetadata missingAtEnd = GitHubReleaseMetadataParser.parseLatest(
                "{\"tag_name\":   "
        );
        GitHubReleaseMetadata missingArrayAtEnd = GitHubReleaseMetadataParser.parseLatest(
                "{\"assets\":   "
        );
        GitHubReleaseMetadata unclosedArray = GitHubReleaseMetadataParser.parseLatest(
                "{\"assets\":[{\"name\":\"dangling.apk\",\"browser_download_url\":\"https://example/dangling\"}"
        );
        GitHubReleaseMetadata nestedArray = GitHubReleaseMetadataParser.parseLatest(
                "{\"assets\":[[],{\"name\":\"nested.apk\",\"browser_download_url\":\"https://example/nested\"}]}"
        );
        GitHubReleaseMetadata emptyAssets = GitHubReleaseMetadataParser.parseLatest(
                "{\"assets\":[]}"
        );
        GitHubReleaseMetadata skippedAssets = GitHubReleaseMetadataParser.parseLatest(
                "{"
                        + "\"tag_name\":\"v0.4.3\","
        ...
        GitHubReleaseMetadata noColonBeforeEnd = GitHubReleaseMetadataParser.parseLatest("{\"tag_name\"   ");
        GitHubReleaseMetadata trailingBackslashAndShortUnicode = GitHubReleaseMetadataParser.parseLatest(
                "{\"html_url\":\"trail\\\\ short\\"
                        + "u\"}"
        );
        GitHubReleaseMetadata terminalBackslash = GitHubReleaseMetadataParser.parseLatest("{\"html_url\":\"trail" + "\\");
        GitHubReleaseMetadata strayObjectClose = GitHubReleaseMetadataParser.parseLatest("{\"assets\":[}]}");
    ...
    @Test
    public void objectValueScannerIgnoresStrayClosingBrace() {
        List<String> values = GitHubReleaseMetadataParser.objectValues("}");
