...
public final class GitHubReleaseMetadataParser {
    ...
    private GitHubReleaseMetadataParser() {
    }

    public static GitHubReleaseMetadata parseLatest(String json) {
        String safeJson = json == null ? "" : json;
        String tag = stringValue(safeJson, KEY_TAG_NAME);
        String html = stringValue(safeJson, KEY_HTML_URL);
        List<GitHubReleaseMetadata.ReleaseAsset> assets = new ArrayList<>();
        ...
        for (String assetJson : objectValues(assetsJson)) {
            ...
            if (!name.isEmpty() && !url.isEmpty()) {
                assets.add(new GitHubReleaseMetadata.ReleaseAsset(name, url));
            }
        }
        return new GitHubReleaseMetadata(tag, html, assets);
    }

    static List<String> objectValues(String arrayJson) {
