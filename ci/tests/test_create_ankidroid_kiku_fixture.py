...
class CreateAnkiDroidKikuFixtureTest(unittest.TestCase):
    ...
    def test_generated_collection_has_expected_schema_counts_and_indexes(self) -> None:
        ...
        self.assertEqual(2, db.execute("SELECT COUNT(*) FROM notes").fetchone()[0])
        self.assertEqual(2, db.execute("SELECT COUNT(*) FROM cards").fetchone()[0])
    ...
    def test_notes_use_anki_field_separator_tags_and_deterministic_checksums(self) -> None:
        ...
        self.assertEqual([1700000000000, 1700000000000], [note[2] for note in notes])
        self.assertEqual([" kiku_ci ", " kiku_ci "], [note[3] for note in notes])
        self.assertEqual(["", ""], [note[7] for note in notes])
        for note in notes:
    ...
    def test_cards_cover_suspended_and_active_provider_rows_with_fsrs_payloads(self) -> None:
        ...
        self.assertEqual((1700000001002, 1700000000002, 1, 0, 2, 2, 1, 7, 2500, 9, 1), cards[1][:11])
