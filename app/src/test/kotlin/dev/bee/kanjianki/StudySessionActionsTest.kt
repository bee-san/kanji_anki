...
class StudySessionActionsTest {
    ...
    fun plannedStudySessionInitializesPlanBeforeChoosingSession() {
        ...
        val plannedSession = requireNotNull(session)
        assertTrue(tracker.pendingPlannedSessionTaskKeys().contains("${plannedSession.taskType}:${plannedSession.item!!.kanji}"))
    ...
    fun plannedStudySessionRequeuesDueLearningRepeatBeforeRemainingPlan() {
        ...
        assertNotNull(session)
        val finishedSession = requireNotNull(session)
        assertEquals("裂", finishedSession.item!!.kanji)
        assertEquals("kanji_meaning", finishedSession.taskType)
    }

    private fun item(kanji: String): RecordsStudyModels.StudyItem =
        RecordsStudyModels.StudyItem(kanji, "review", 1000L, 1.0, 2.0, 1, 0, 0, 0, "", 1000L)
            .copyBuilder()
