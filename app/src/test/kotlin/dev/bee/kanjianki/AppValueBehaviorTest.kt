...
class AppValueBehaviorTest {
    ...
    @Test
    fun meaningKanjiChoiceCopyUsesDictionaryMeaningWhenAvailable() {
        val lookup = DictionaryLookup.fromKanjiEntries(
            listOf(
    ...
    fun studyCueTextsHandleNullRowsAndExamples() {
        ...
        val wordReading = session("読", false, BridgeScheduler.TASK_WORD_READING, "", "")
        val emptyMeaning = example("言語", "ゴ", "")
        val nullExpression = example(null, null, "")

        assertEquals(listOf("Collection clue"), StudyCueTexts.answerLines(DictionaryLookup.empty(), noRow, null, false))
        assertEquals(
    ...
    @Test
    fun studyReviewRequestsDistinguishWritingFailureOutcomes() {
        val writingSession = session("清", true, BridgeScheduler.TASK_WRITE_KANJI)
        val noInk = WritingAnalysisEngine.noInk()
        val recognitionError = WritingAnalysisEngine.recognitionError()

        val noInkMapped = StudyReviewRequestPolicy.from(writingSession, StudyReviewWritingOutcome.from(noInk), 1, "good", false)
        val errorMapped = StudyReviewRequestPolicy.from(writingSession, StudyReviewWritingOutcome.from(recognitionError), 3, "good", false)

        assertFalse(noInkMapped.request().writingPassed)
        assertFalse(noInkMapped.request().writingClean)
        assertEquals("again", noInkMapped.ratingCode())
        assertFalse(errorMapped.request().writingPassed)
