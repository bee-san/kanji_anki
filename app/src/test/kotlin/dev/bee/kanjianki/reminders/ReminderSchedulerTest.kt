...
class ReminderSchedulerTest {
    ...
    fun showReminderNotificationStopsBeforeBuildingWhenNotificationsAreBlocked() {
        val services = FakeReminderServices().apply { runtimePermission = false }
    ...
    fun bootReceiverActionOrEmptyReadsOnlyPresentSources() {
        assertEquals("", BootReminderReceiver.actionOrEmpty(null) { error("Null sources must not be read") })
        assertEquals(
    ...
    private class FakeReminderServices : ReminderScheduler.ReminderServices {
        ...
        override fun reminderChannelImportance(): Int? = channelImportance
