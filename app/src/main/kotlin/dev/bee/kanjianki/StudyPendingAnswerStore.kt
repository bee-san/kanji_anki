...
internal class StudySessionRecoveryStore(
    ...
    private fun validPending(snapshot: StudyPendingAnswerSnapshot): Boolean {
        val answerSignature = snapshot.answerSignature
        val schedulerRevision = snapshot.schedulerRevision
        val repairId = snapshot.repairId
        val repairAttempts = snapshot.repairAttempts
        val identityComplete = (answerSignature == null) == (schedulerRevision == null)
        val repairIdentityComplete = (repairId == null) == (repairAttempts == null)
        val repairIdentityValid = if (snapshot.taskType == MainActivityBase.TASK_REPAIR_WRITING) {
            repairId == null || repairId > 0L && repairAttempts?.let { it >= 0 } == true
        } else {
            repairId == null && repairAttempts == null
        }
        ...
        return validText(snapshot.feedback.sessionToken, MAX_TOKEN_CHARS, allowBlank = false) &&
            ...
            (answerSignature == null || validText(answerSignature, MAX_SIGNATURE_CHARS, allowBlank = true)) &&
            (schedulerRevision == null || schedulerRevision >= 0L)
    }
