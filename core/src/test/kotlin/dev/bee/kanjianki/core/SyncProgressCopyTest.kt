...
class SyncProgressCopyTest {
    ...
    fun japaneseLocaleTranslatesSyncProgressCopy() {
        withLocale(Locale.JAPANESE) {
            ...
            assertEquals(
                "カード数がわかる前にノートを読み込んでいます。",
                SyncProgressCopy.stageBody(SyncProgressCopy.Stage.READING_NOTES),
            )
            assertEquals("カードのスキャンを準備しています。", SyncProgressCopy.stageBody(SyncProgressCopy.Stage.SCANNING_CARDS))
            assertEquals(
                "AnkiDroidの読み込みが完了しました。取り込んだカードをローカルで処理しています。",
                SyncProgressCopy.stageBody(SyncProgressCopy.Stage.PROCESSING_IMPORTED_CARDS),
            )
            assertEquals(
                "Ankiのスナップショットと取り込み証拠を保存しています。",
                SyncProgressCopy.stageBody(SyncProgressCopy.Stage.SAVING_LOCAL_DATA),
            )
            assertEquals("練習キューを保存しています。", SyncProgressCopy.stageBody(SyncProgressCopy.Stage.BUILDING_PRACTICE_QUEUE))
            assertEquals("アーカイブした停止カードを更新しています。", SyncProgressCopy.stageBody(SyncProgressCopy.Stage.ARCHIVING_IMPORTED_CARDS))
            assertEquals("カードのスキャンを準備しています。", SyncProgressCopy.stageBody(null))

            assertEquals("0 / 10 枚スキャン済み", SyncProgressCopy.cardProgressText(-2, 10))
            assertEquals("7 / 12 枚スキャン済み", SyncProgressCopy.cardProgressText(7, 12))
            assertEquals("7 / 0 枚スキャン済み", SyncProgressCopy.cardProgressText(7, -12))

            assertEquals("カードをスキャン中です。", SyncProgressCopy.scanRateText(SyncProgressCopy.Stage.SCANNING_CARDS, 0, 20, 1000))
            assertEquals(
                "2.0 枚/秒 - 残り時間を計算中",
                SyncProgressCopy.scanRateText(SyncProgressCopy.Stage.SCANNING_CARDS, 2, 20, 1000),
            )
            ...
            assertEquals("1 秒", SyncProgressCopy.shortDuration(0))
            assertEquals("59 秒", SyncProgressCopy.shortDuration(59_000))
            assertEquals("1 分", SyncProgressCopy.shortDuration(60_000))
            assertEquals("2 分", SyncProgressCopy.shortDuration(90_000))
            assertEquals("1 時間", SyncProgressCopy.shortDuration(3_600_000))
            assertEquals("2 時間", SyncProgressCopy.shortDuration(5_400_000))
        }
    }
