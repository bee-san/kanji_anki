...
class SettingsSectionTextCopyTest {
    ...
    fun sectionLabelsPreserveFormatting() {
        ...
        assertEquals("Configure fields, filters, range, and sync.", SettingsSectionTextCopy.settingsAnkiSourceBody())
        assertEquals("Study settings", SettingsSectionTextCopy.settingsStudyBehaviorTitle())
        assertEquals("Tune new cards, timing, workload, and ladder.", SettingsSectionTextCopy.settingsStudyBehaviorBody())
        assertEquals("Automation", SettingsSectionTextCopy.settingsAutomationTitle())
        assertEquals("Manage reminders and app updates.", SettingsSectionTextCopy.settingsAutomationBody())
        assertEquals("Display & data", SettingsSectionTextCopy.settingsReferenceDataTitle())
        assertEquals("Manage dictionaries, stroke data, fonts, and credits.", SettingsSectionTextCopy.settingsReferenceDataBody())
    ...
    fun sectionLabelsTranslateToJapaneseLocale() {
        ...
        try {
            ...
            assertEquals("フィールド、フィルター、範囲、同期を設定。", SettingsSectionTextCopy.settingsAnkiSourceBody())
            assertEquals("学習設定", SettingsSectionTextCopy.settingsStudyBehaviorTitle())
            assertEquals("新規カード、タイミング、負荷、ラダーを調整。", SettingsSectionTextCopy.settingsStudyBehaviorBody())
            assertEquals("自動化", SettingsSectionTextCopy.settingsAutomationTitle())
            assertEquals("リマインダーとアプリ更新を管理。", SettingsSectionTextCopy.settingsAutomationBody())
            assertEquals("表示とデータ", SettingsSectionTextCopy.settingsReferenceDataTitle())
            assertEquals("辞書、ストロークデータ、フォント、クレジットを管理。", SettingsSectionTextCopy.settingsReferenceDataBody())
