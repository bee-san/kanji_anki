...
class KaniOutcomePolicyTest {
    @Test
    fun outcomeStatsUseOnlyKanjiWithBothSnapshots() {
        val stats = KaniOutcomePolicy.summarize(
            listOf(
    ...
    @Test
    fun publicMetricsClampAndDefaultUnsafeInput() {
        val empty = KaniOutcomePolicy.OutcomeStats.empty()
        assertEquals(0, empty.weakKanjiImproved().improvedCount())
        assertEquals(0, empty.matureSupportGained().gainedSupportCount())

        val nullBacked = KaniOutcomePolicy.OutcomeStats(null, null, null)
        ...
        val weak = KaniOutcomePolicy.WeakKanjiImprovedMetric(-1, -2.0, -3.0, null)
        ...
        val support = KaniOutcomePolicy.MatureSupportGainedMetric(-1, -2, -3, null)
        ...
        val legacySupport = KaniOutcomePolicy.MatureSupportGainedMetric(2, 1, emptyList())
    ...
    @Test
    fun evidenceAndExampleValuesNormalizeUnsafeInput() {
        val evidence = outcome(null, snapshot(-1, -2), snapshot(-3, -4))
        val before = evidence.before()!!
        val after = evidence.after()!!
        assertEquals("", evidence.kanji())
        assertEquals(0, before.weaknessScore())
        assertEquals(0, before.matureSupportCount())
        assertEquals(0, after.weaknessScore())
        assertEquals(0, after.matureSupportCount())

        val improvement = KaniOutcomePolicy.KanjiImprovement(null, -0.2, -0.1)
        ...
        val gain = KaniOutcomePolicy.KanjiSupportGain(null, -1, -2)
    ...
    private fun outcome(
        kanji: String?,
        before: KaniOutcomePolicy.OutcomeSnapshot?,
        after: KaniOutcomePolicy.OutcomeSnapshot?,
    ): KaniOutcomePolicy.OutcomeEvidence =
        KaniOutcomePolicy.OutcomeEvidence(kanji, before, after)

    private fun snapshot(weaknessScore: Int, matureSupportCount: Int): KaniOutcomePolicy.OutcomeSnapshot =
        KaniOutcomePolicy.OutcomeSnapshot(weaknessScore, matureSupportCount)
}
