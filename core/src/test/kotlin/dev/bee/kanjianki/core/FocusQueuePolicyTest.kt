...
class FocusQueuePolicyTest {
    ...
    private fun kanji(entries: List<FocusQueuePolicy.QueueEntry>): List<String> = entries.map { it.row.kanji }
    ...
    private fun review(kanji: String, dueAtMillis: Long): RecordsStudyModels.StudyItem =
        item(kanji, StudyLadderRules.STATE_REVIEW, dueAtMillis, 1)

    private fun item(kanji: String, state: String, dueAtMillis: Long, totalReviews: Int): RecordsStudyModels.StudyItem =
        RecordsStudyModels.StudyItem(kanji, state, dueAtMillis, 1.0, 5.0, totalReviews, 0, 0, 1, null, 0L)
}
