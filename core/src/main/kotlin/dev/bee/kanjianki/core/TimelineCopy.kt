...
object TimelineCopy {
    ...
    fun statusText(timeline: RecordsStudyModels.KanjiRecoveryTimeline, nowMillis: Long): String {
        ...
        if (item != null && StudyLadderRules.STATE_RETIRED == item.state) {
            return localizedText("Retired by Anki support", "Ankiの支援で終了")
        }
        if (item != null && item.dueAtMillis > nowMillis) {
            return localizedText("Resting until review", "復習まで待機中")
        }
    ...
    fun sourceLine(event: RecordsImportModels.KanjiTimelineEvent): String {
        ...
        if (event.sourceReading.isEmpty()) {
            return localizedText("Source: " + event.sourceExpression, "出典: " + event.sourceExpression)
        }
        return localizedText(
            "Source: " + event.sourceExpression + "  " + event.sourceReading,
            "出典: " + event.sourceExpression + "  " + event.sourceReading,
        )
    }
    ...
    fun studyStateDetail(retired: Boolean, matureSupportCount: Int?, target: Int): String {
        if (retired) {
            return if (matureSupportCount == null) {
                ...
                supportDetail(
                    localizedText("Mature Anki support met the target", "成熟Ankiサポートは目標に達しました"),
                    matureSupportCount,
                    target,
                )
            }
        ...
        return if (matureSupportCount == null) {
            ...
            supportDetail(
                localizedText("Mature Anki support fell below target", "成熟Ankiサポートは目標を下回りました"),
                matureSupportCount,
                target,
            )
        }
        ...
        if (RATING_AGAIN == safeRating) {
            return if (request.writingRequired) {
                localizedText(
                    ...
                    "書き取りに失敗したため、Kaniは再表示します。",
                )
            } else {
                localizedText(
                    "Recall missed; Kani will show it again.",
                    "想起に失敗したため、Kaniは再表示します。",
                )
            }
        }
        if (request.writingRequired) {
            return if (request.writingPassed) {
                localizedText(
                    ...
                    "書き取りは合格し、" + safeRating + "として評価されました。",
                )
            } else {
                localizedText(
                    "Writing was not passed and was rated " + safeRating + ".",
                    "書き取りは合格せず、" + safeRating + "として評価されました。",
                )
            }
        }
        return localizedText(
            "Recall review was rated " + safeRating + ".",
            "想起レビューは" + safeRating + "として評価されました。",
        )
    }
    ...
    fun supportDetail(prefix: String, matureSupportCount: Int, target: Int): String {
        return if (isJapaneseLocale()) {
            "$prefix: 成熟サポート $matureSupportCount / 目標 $target。"
        } else {
