...
object SettingsSectionTextCopy {
    ...
    @JvmStatic
    fun settingsAnkiSourceBody(): String = localizedText("Sources, filters, range.", "ソース、フィルター、範囲を選ぶ。")

    @JvmStatic
    fun settingsStudyBehaviorTitle(): String = localizedText("Study settings", "学習設定")

    @JvmStatic
    fun settingsStudyBehaviorBody(): String = localizedText("New cards, timing, workload, ladder.", "新規カード、タイミング、負荷、ラダーを設定。")

    @JvmStatic
    fun settingsAutomationTitle(): String = localizedText("Automation", "自動化")

    @JvmStatic
    fun settingsAutomationBody(): String = localizedText("Reminders, sync, updates.", "リマインダー、同期、更新を管理。")

    @JvmStatic
    fun settingsReferenceDataTitle(): String = localizedText("Display & data", "表示とデータ")

    @JvmStatic
    fun settingsReferenceDataBody(): String = localizedText("Dictionaries, strokes, fonts, credits.", "辞書、ストローク、フォント、クレジットを管理。")
